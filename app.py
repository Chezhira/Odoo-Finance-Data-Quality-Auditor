...
def main() -> None:
    st.markdown(
        ...
        .hero p { margin: 0.35rem 0 0; color: #475569; font-size: 1rem; }
        .kpi {
    ...
    st.markdown(
        ...
        <div class="hero">
            ...
            <p>Finance controls and audit-readiness review for Odoo-compatible accounting exports.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    sample_data_dir = Path(
        st.sidebar.text_input("Sample data folder", value=str(SAMPLE_DATA_DIR))
    )
...
def _count_by(rows: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    return rows[column].value_counts().rename_axis(column).reset_index(name="count").set_index(column)
